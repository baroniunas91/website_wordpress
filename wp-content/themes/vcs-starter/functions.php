...
//IMG Dydziai
// add_image_size('dyzdio-pavadinimas', plotis, aukstis, ar karpyti);
add_image_size('logo', 31, 25, false);
add_image_size('post-block', 680, 440, true);
add_image_size('skills-block', 700, 570, true);
add_image_size('view-eye', 50, 32, true);
...
function theme_scripts(){
    ...
    if ( !is_admin() ) {
		...
		// https://kit.fontawesome.com/d7d4d43dac.js
		// assets/scripts/jquery.js
		// assets/scripts/jquery.fancybox.min.js
		// assets/scripts/tiny-slider.js
		// assets/scripts/custom.js

        wp_deregister_script('jquery');
		wp_register_script(
			'jquery', 
			THEME_FOLDER . '/assets/scripts/jquery.js', 
			false, 
			false, 
			true
		);
		...
		wp_register_script(
			'fancybox', 
			THEME_FOLDER . '/assets/scripts/jquery.fancybox.min.js',
			array('fa'), 
			false, 
			true
		);
		...
		wp_register_script(
			'tiny-slider', 
			THEME_FOLDER . '/assets/scripts/tiny-slider.js',
			array('fancybox'), 
			false, 
			true
		);
...
function theme_stylesheets(){
	...
	if( defined('THEME_FOLDER') ) {
		...
		wp_register_style(
			'gf', 
			'https://fonts.googleapis.com/css?family=Montserrat:500|Nunito+Sans|Work+Sans&display=swap&subset=latin-ext', 
			array(), 
			false, 
			'all'
		);
		wp_register_style(
			'jquery', 
			THEME_FOLDER . '/assets/css/jquery.fancybox.min.css', 
			array('gf'), 
			false, 
			'all'
		);
		wp_register_style(
			'tiny-slider', 
			THEME_FOLDER . '/assets/css/tiny-slider.css', 
			array('jquery'), 
			false, 
			'all'
		);
